...
int merge_union(char **strings, long left, long middle, long right)
{
	...
	long i = 0, j = 0;
	for ( ; left <= right ; left++ )
		if ( (j >= right_length) || ((i < left_length) && (j < right_length) && (strcmp(buf_array[left_length + j], buf_array[i]) >= 0)) )
...
int radix_in_buckets(char **strings, char **copy, long left, long right, long depth)
{
	...
	char *real_symbols = calloc(diff, sizeof(char));
	long *order_symbols = calloc(ALPHABET, sizeof(long));
	for ( buf = ALPHABET - 1; different_symbols != 0; buf--)
		if (symbols[buf] != 0)
			real_symbols[--different_symbols] = (char) buf;
	order_symbols[real_symbols[0]] = left;
	for ( buf = 1; buf < diff; buf++ )
			order_symbols[real_symbols[buf]] = order_symbols[real_symbols[buf - 1]] + symbols[real_symbols[buf - 1]];
	for ( buf = left ; buf <= right; buf++ )
		copy[order_symbols[strings[buf][depth]]++] = strings[buf];
	for ( buf = left ; buf <= right; buf++ )
	...
	for ( buf = 0; buf < diff; buf++ )
		if ( symbols[real_symbols[buf]] > 1 && real_symbols[buf] != '\0' && real_symbols[buf] != '\n')
			radix_in_buckets(strings, copy, buf > 1 ? order_symbols[real_symbols[buf - 1]] : left, order_symbols[real_symbols[buf]] - 1, depth + 1);
	free(real_symbols);
	free(symbols);
	free(order_symbols);
